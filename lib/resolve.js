...
export async function resolveStream(input, origin) {
  ...
  console.log(`[resolve] meta ${uri}`)
  const meta = await fetchStreamMeta(uri)
  if (meta.error) {
    console.log(`[resolve] meta failed ${meta.error}`)
    return fail('meta', meta.error, { uri, contentPath })
  }
  ...
  console.log(`[resolve] decrypt ${embedPath}`)
  try {
    const { streamUrl } = await resolveEmbedStreamUrl(embedPath)
    console.log(`[resolve] ok ${streamUrl}`)
