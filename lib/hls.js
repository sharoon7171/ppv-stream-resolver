...
function rewriteM3u8(text, baseUrl, embedPath) {
  ...
  for (const line of lines) {
      ...
      if (trimmed.startsWith('#EXT-X-MAP:')) {
        out.push(
          trimmed.replace(/URI="([^"]+)"/, (_, uri) => {
            ...
            return `URI="${proxyQuery(abs, embedPath)}"`
          }),
        )
      } else {
        out.push(line)
    ...
    out.push(proxyQuery(abs, embedPath))
...
async function proxySegment(targetUrl) {
  ...
  if (isHtmlError(upstream.body)) throw new Error('upstream html error')
  return { status: 200, headers: segmentHeaders(), body: segmentBody(upstream.body) }
}

export async function proxyHlsRequest(targetUrl, embedPath) {
  if (!isM3u8Resource(targetUrl)) return proxySegment(targetUrl)
  ...
  if (isPlaylist(targetUrl, contentType, upstream.body)) {
    return {
      ...
      body: rewriteM3u8(upstream.body.toString('utf8'), targetUrl, embedPath),
    }
  }
  if (isHtmlError(upstream.body)) throw new Error('upstream html error')
  return { status: 200, headers: segmentHeaders(), body: segmentBody(upstream.body) }
...
export async function writeProxyHlsResponse(res, targetUrl, embedPath) {
  const proxied = await proxyHlsRequest(targetUrl, embedPath)
